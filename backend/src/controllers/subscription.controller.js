...
export const subscribe = async (req, res) => {
    try {
        ...
        const result = await subscriptionService.subscribeChannel(String(req.user.id), channelId);
        res.status(200).json({
            success: true,
...
export const unsubscribe = async (req, res) => {
    try {
        ...
        const result = await subscriptionService.unsubscribeChannel(String(req.user.id), channelId);
        res.status(200).json({
            success: true,
...
export const getUserSubscriptions = async (req, res) => {
    try {
        const result = await subscriptionService.getUserSubscriptions(String(req.user.id));
        res.status(200).json({
            success: true,
